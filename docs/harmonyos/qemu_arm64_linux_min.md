...
```json
{
  ...
  "subsystems": [
    ...
    },
    {
      ...
      "components": [
        {
          "component": "qemu_arm_linux_chipset",
          "features": []
        }
      ]
    }
...
elf_file="out/qemu-arm64-linux/packages/phone/images"
...
[ x"${qemu_setup_network}" == x ] && qemu-system-aarch64 ${qemu_option} -append "${kernel_bootargs}"
```

### 3、build/subsystem_components_whitelist.json
...
# Linux/arm64 5.10.210 Kernel Configuration
#
...
# CONFIG_SCHED_RUNNING_AVG is not set
# CONFIG_CPU_ISOLATION_OPT is not set
...
#
# end of Related Thread Group

# CONFIG_SCHED_EAS is not set
...
# CONFIG_MEMORY_MONITOR is not set
# CONFIG_PAGE_TRACING is not set
# CONFIG_RECLAIM_ACCT is not set
CONFIG_SPARSEMEM=y
...
# CONFIG_LOWMEM is not set
# CONFIG_LMKD_DBG is not set
# CONFIG_RSS_THRESHOLD is not set
...
# CONFIG_TCP_NATA_URC is not set
# CONFIG_TCP_NATA_STL is not set
# CONFIG_LOWPOWER_PROTOCOL is not set
# CONFIG_TCP_SOCK_DESTROY is not set
...
# CONFIG_DMABUF_PROCESS_INFO is not set
...
# CONFIG_BINDER_TRANSACTION_PROC_BRIEF is not set
...
# CONFIG_DRIVERS_HDF_PLATFORM_CLOCK is not set
...
# CONFIG_DRIVERS_HDF_WIFI_BUS is not set
...
+            cmd = ['timeout', '-s', '9', '600s', npm, 'install', '--registry', args.npm_registry, '--cache', npm_cache_dir]
